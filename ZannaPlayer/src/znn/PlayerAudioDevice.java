 ...
 * @author Mat McGowan
 */
public class PlayerAudioDevice extends AudioDeviceBase {
    ...
    private float getGain(FloatControl volControl, float gain) {
        float min = volControl.getMinimum();
        float max = volControl.getMaximum();
        float r = ((max - min) * gain) - (min < 0 ? min * -1 : min);

        return Math.min(Math.max(r, volControl.getMinimum()), volControl.getMaximum());
    }

    public boolean setLineGain(float gain) {
        if (source != null) {
            FloatControl volControl = (FloatControl) source.getControl(FloatControl.Type.MASTER_GAIN);
            float newGain = getGain(volControl, gain);
    ...
    public void fadeIn(float from, float to, final int seconds) {
        if (source != null) {
            final FloatControl control = (FloatControl) source.getControl(FloatControl.Type.MASTER_GAIN);
            
            from = getGain(control, from);
            to = getGain(control, to);
